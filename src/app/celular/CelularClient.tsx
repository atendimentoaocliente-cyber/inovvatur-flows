...
function Tiques({ tique, grupo }: { tique: Tique; grupo: boolean }) {
  ...
  return (
    <span
      title={
        grupo
          ? 'Enviada: está no grupo. Em grupo o WhatsApp não devolve "entregue/lida" por pessoa para o disparador.'
          : 'Enviada ao WhatsApp'
      }
    >
      ✓
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {conversas === null ? (
              ...
              <p className="px-4 py-6 text-sm" style={{ color: WA.cinza }}>
                {filtro === 'sendflow'
                  ? 'Nenhum grupo cadastrado no disparador aparece neste número. Sincronize em Conexões.'
                  : 'Nada por aqui.'}
              </p>
            ) : (
              visiveis.map((c) => (
      ...
      <header className="flex items-center gap-3 px-3 py-2.5" style={{ background: WA.barra }}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="truncate text-[12.5px]" style={{ color: WA.cinza }}>
            {conversa.grupo
              ? conversa.sendflow
                ? `grupo do disparador · ${conversa.ativo ? 'ativo para disparo' : 'inativo para disparo'}`
                : 'grupo fora do disparador'
              : conversa.telefone
                ? `+${conversa.telefone}`
                : 'contato'}
          </div>
